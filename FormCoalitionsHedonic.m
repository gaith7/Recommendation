function [ part, ProvidedAvailability ] = FormCoalitionsHedonic( partition, beliefs, availability )
    ...
    for j=1:part.size()-1
        ...
        if(tf==0)
           ...
           for c=1:length(coalition)
               if(beliefs(coalition(c)<0.5))
                   WorthCoalition=WorthCoalition-999999999;
               else
                   WorthCoalition=WorthCoalition+beliefs(coalition(c));
                   availabilityCoalition=availabilityCoalition+availability(coalition(c));
               end
           end  
           
           for c=1:length(coalition)
            %[ payoff ] = ComputePayoffAvailability(coalition, WorthCoalition, beliefs(coalition(c)));
            WorthCoalitions.add(coalition(c));
            WorthCoalitions.add(coalition);
            WorthCoalitions.add(WorthCoalition);
            
            li.add(coalition(c));
            li.add(coalition);
            li.add(WorthCoalition); 
