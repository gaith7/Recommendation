function [ part, ProvidedAvailability ] = CloudFederationsHedonic( partition, price, cost )

Worth=zeros(size(price,1),1);

for i=1:price
    numberOfConsumedServices=(15-1).*rand(1,1) + 1;
    Worth(i)=numberOfConsumedServices*(price(i)-cost(i));
end
    ...
    for j=1:part.size()-1
        ...
        if(tf==0)
           ...
           for c=1:length(coalition)
               ...
               UtilityCoalition=UtilityCoalition+Worth(coalition(c));
           end  
           ...
           for c=1:length(coalition)
            ...
            AvailabilityCoalitions.add(UtilityCoalition);
